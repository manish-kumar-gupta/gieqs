...
function lightspeed_get_template( $block_type, $block_order )
{
	$alternate 	= ( $block_order % 2 == 0 && in_array( $block_type, array( 'content-with-media' ) ) ) ? '-alternate' : '';
	$fallback 	= get_option( 'areoi-lightspeed-blocks-' . $block_type . $alternate );
	if ( !$fallback ) $fallback = get_option( 'areoi-lightspeed-blocks-' . $block_type );
...
function lightspeed_get_contrast_color( $col )
{
    ...
    if ( !is_array( $col ) && str_starts_with( $col, '#' ) ) {
    	$R1 = hexdec(substr($col, 1, 2));
        $G1 = hexdec(substr($col, 3, 2));
        $B1 = hexdec(substr($col, 5, 2));
    } else {
    	$R1 = $col['r'];
	    $G1 = $col['g'];
	    $B1 = $col['b'];
...
{
	$logos = array(
		'dark' => get_option( 'areoi-lightspeed-company-logo-dark', null ),
		'light' => get_option( 'areoi-lightspeed-company-logo-light', null ),
	);

	$icons = array(
		'dark' => get_option( 'areoi-lightspeed-company-icon-dark', null ),
		'light' => get_option( 'areoi-lightspeed-company-icon-light', null ),
	);
...
function lightspeed_get_content( $heading_level, $default_align )
{
	$content = '';
...
function lightspeed_content( $heading_level, $default_align )
{
	echo lightspeed_get_content( $heading_level, $default_align );
}
...
function lightspeed_get_heading( $level, $item = null, $level_class = '' )
{
	...
	$heading_template = '';
	if ( $level < 3 ) $heading_template = get_option( 'areoi-lightspeed-styles-headings', null );
	...
	if ( $heading ) {
		...
		$include_icon = get_option( 'areoi-lightspeed-styles-heading-icon', null );
...
{	
	$selected_icon = get_option( 'areoi-lightspeed-styles-btn-icon', null );
...
{
	echo '<svg class="areoi-square-spacer" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 5 5" style="enable-background:new 0 0 5 5;" xml:space="preserve"></svg>
	';
}
...
function lightspeed_item( $item, $is_post = false )
{	
	if ( $is_post ) {
		global $post;
		...
		if ( !$media && get_option( 'areoi-lightspeed-company-include-lightspeed', false ) ) {
	...
	?>
	<div class="d-flex flex-column h-100 position-relative <?php echo $stretched_link ? 'areoi-has-url' : '' ?>">
		<?php if ( $media ) : ?>
		...
		<?php endif; ?>
		<div class="flex-grow-1 areoi-item-content card-body <?php echo $background_color ? $background_color . ' ' : 'ps-0 pe-0' ?> <?php echo !$media ? 'pt-0' : '' ?>">
			<?php echo $heading ?>
...
function lightspeed_contact( $color = '', $class = '', $include_address = true, $format_address = false )
{
	$company_address 	= lightspeed_get_attribute( 'company_address', get_option( 'areoi-lightspeed-company-address', null ) );
	$company_phone 		= lightspeed_get_attribute( 'company_phone', get_option( 'areoi-lightspeed-company-phone', null ) );
	$company_email 		= lightspeed_get_attribute( 'company_email', get_option( 'areoi-lightspeed-company-email', null ) );
	...
	<?php if ( !lightspeed_get_attribute( 'exclude_company', false ) ) : ?>
		...
		<?php if ( $company_phone ) : ?>
			<p class="<?php echo $class ?> <?php echo $color ?>">
				<svg class="me-1" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="currentColor"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M6.54 5c.06.89.21 1.76.45 2.59l-1.2 1.2c-.41-1.2-.67-2.47-.76-3.79h1.51m9.86 12.02c.85.24 1.72.39 2.6.45v1.49c-1.32-.09-2.59-.35-3.8-.75l1.2-1.19M7.5 3H4c-.55 0-1 .45-1 1 0 9.39 7.61 17 17 17 .55 0 1-.45 1-1v-3.49c0-.55-.45-1-1-1-1.24 0-2.45-.2-3.57-.57-.1-.04-.21-.05-.31-.05-.26 0-.51.1-.71.29l-2.2 2.2c-2.83-1.45-5.15-3.76-6.59-6.59l2.2-2.2c.28-.28.36-.67.25-1.02C8.7 6.45 8.5 5.25 8.5 4c0-.55-.45-1-1-1z"/></svg>
...
function lightspeed_social( $color = '' )
{
	$social_facebook 	= lightspeed_get_attribute( 'social_facebook', get_option( 'areoi-lightspeed-company-facebook', null ) );
	$social_twitter 	= lightspeed_get_attribute( 'social_twitter', get_option( 'areoi-lightspeed-company-twitter', null ) );
	$social_instagram 	= lightspeed_get_attribute( 'social_instagram', get_option( 'areoi-lightspeed-company-instagram', null ) );
		...
		<div class="areoi-social d-flex <?php echo $color ?>">
			...
			<?php if ( $social_youtube ) : ?>
				<a href="<?php echo $social_youtube ?>" target="_blank" class="<?php echo $color ?> areoi-has-url areoi-has-url-small">
					<svg class="ms-1" xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 0 24 24" width="30px" fill="currentColor"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M20.5,3.5C20.2,3.2,19.7,3,19.2,3H4.8C4.3,3,3.8,3.2,3.5,3.5C3.1,3.9,3,4.3,3,4.8v14.5c0,0.5,0.2,0.9,0.5,1.2  C3.8,20.8,4.3,21,4.8,21h14.4c0.5,0,0.9-0.2,1.3-0.5c0.4-0.4,0.5-0.8,0.5-1.2V4.8C21,4.3,20.9,3.9,20.5,3.5z M15.8,12.4l-6.4,3.3  V8.2l7.3,3.8L15.8,12.4z"/></svg>
				</a>
			<?php endif; ?>
		</div>
	<?php endif;
...
function lightspeed_media_carousel( $medias, $type = 'square', $include_mask = true )
{
	...
	<div id="<?php lightspeed_block_id() ?>-media-carousel" class="carousel slide" data-bs-ride="carousel">
		...
		<div class="carousel-inner <?php echo $include_mask ? 'areoi-has-mask' : '' ?>">
			<?php foreach ( $medias as $gallery_key => $media ) : ?>
...
function lightspeed_tabs( $items )
{
?>
	<ul class="nav nav-tabs border-0 nav-justified" id="<?php echo lightspeed_get_block_id() . '-tabs' ?>" role="tablist">
		<?php 
...
?>
	<div class="row row-cols-1 justify-content-between align-items-center">
		<div class="col col-lg-4 <?php echo lightspeed_get_attribute( 'alignment', 'start' ) == 'end' ? 'order-lg-1' : '' ?>">
			<ul class="nav nav-pills flex-column border-0 nav-justified" id="<?php echo lightspeed_get_block_id() . '-tabs' ?>" role="tablist">
				...
				?>
					...
					>
						<button 
						class="nav-link h3 bg-transparent <?php echo $heading_color . ' ' . $border_color ?> <?php echo $item_key == 0 ? 'active' : '' ?>" 
						id="<?php echo lightspeed_get_block_id() . '-' . $item_key ?>-tab" 
						data-bs-toggle="tab" 
						data-bs-target="#<?php echo lightspeed_get_block_id() . '-' . $item_key ?>" 
						type="button" 
