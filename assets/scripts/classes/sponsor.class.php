...
Class sponsor {
     ...
     *
     */
	public function New_sponsor($name,$long_name,$logo_src,$created){
		$this->name = $name;
	...
	public function Load_from_key($key_row){
		$result = $this->connection->RunQuery("Select * from sponsor where id = \"$key_row\" ");
...
$q = "Select * from `sponsor` LIMIT " . $x . ", " . $y;
...
$q = "Select * from `sponsor` WHERE `id` = $key";
		$result = $this->connection->RunQuery($q);
							$rowReturn = array();
						$x = 0;
						$nRows = $result->rowCount();
	...
	public function numberOfRows(){
		return $this->connection->TotalOfRows('sponsor');
	}
...
$q = "INSERT INTO `sponsor` ($keys) VALUES ($keys2)";
...
$q = "UPDATE `sponsor` SET $implodeArray WHERE `id` = '$this->id'";
 ...
 return $stmt->rowCount(); 
	...
	public function Delete_row_from_key($key_row){
		$result = $this->connection->RunQuery("DELETE FROM `sponsor` WHERE `id` = $key_row");
	...
	public function GetKeysOrderBy($column, $order){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT id from sponsor order by $column $order");
