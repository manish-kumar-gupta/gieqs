...
INNER JOIN `imagesDraft` as c on b.`image_id` = c.`id` WHERE a.`approved` IS NULL GROUP BY a.`id` ORDER BY a.`created` desc";
	...
	public function writeSelect($svalue1, $svalue2, $tableNameValues){
	 ...
	 public function archiveTableRow ($table, $id){
	 ...
	 public function check_login($email, $pass) {
			...
			$salt = 'westmead';
			$password = trim($pass);
			$p = $this->hash_password($password, $salt);
			//echo $p;
	...
	if ($email && $pass) {
		...
		$q = "SELECT `user_id`, `firstname`, `surname`, `centre`, `access_level` FROM users WHERE `email`='$email' AND `password`='$p'";
		//echo $q;

		$r = $this->connection->RunQuery($q);
	...
	public function copyRecords_SessionItem_video (){
		...
		ORDER BY a.`date`, e.`timeFrom` ASC";
	...
	public function copyRecords_SessionItem_video_specific ($programmeid){
	...
	public function updateSessionItemVideoId($videoid, $sessionItemid){
		...
		if ($result->num_rows == 1){
			...
			echo 'sessionItem ' . $sessionItemid . ' updated with video id ' . $videoid . '';
