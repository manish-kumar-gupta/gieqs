...
class sponsors
{
    ...
    public function Load_records_limit_json($y, $x = 0)
    {
        ...
        if ($nRows > 0) {
            ...
            while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                ...
                $x++;}return json_encode($rowReturn);} else {return false;
        }

	}
	
	public function Load_records_php()
    {
	...
	public function get_sponsor_emails()
    {
        $q = "Select * from `sponsors`";
        $result = $this->connection->RunQuery($q);
        $rowReturn = array();
	...
	public function get_sponsor_emails_optouts_removed()
    {
        $q = "Select * from `sponsors` WHERE `optOut` IS NULL";
        $result = $this->connection->RunQuery($q);
        $rowReturn = array();
        $x = 0;
        $nRows = $result->rowCount();
