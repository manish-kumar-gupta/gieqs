...
Class learningToolSlides {

	private $id; //int(10)
	private $learningToolid; //int(10)
	private $slideid; //int(10)
	private $slideorder; //int(5)
	private $connection;

	public function learningToolSlides(){
     ...
     *
     */
	public function New_learningToolSlides($learningToolid,$slideid,$slideorder){
	...
	public function Load_from_key($key_row){
		$result = $this->connection->RunQuery("Select * from learningToolSlides where id = \"$key_row\" ");
		while($row = $result->fetch_array(MYSQLI_ASSOC)){
			$this->id = $row["id"];
			$this->learningToolid = $row["learningToolid"];
			$this->slideid = $row["slideid"];
			$this->slideorder = $row["slideorder"];
		}return $result; 
	}
	...
	public function Delete_row_from_key($key_row){
		$result = $this->connection->RunQuery("DELETE FROM learningToolSlides WHERE id = $key_row");
return $result; 	}
	...
	public function Save_Active_Row(){
			...
			$q = "UPDATE learningToolSlides SET $implodeArray WHERE id = \"$this->id\"";
	...
	public function Save_Active_RowNulls(){
		$result = $this->connection->RunQuery("UPDATE learningToolSlides set learningToolid = \"$this->learningToolid\", slideid = \"$this->slideid\", slideorder = \"$this->slideorder\" where id = \"$this->id\"");
	return $result; 
	}
	...
	public function Save_Active_Row_as_New(){
	 ...
	 foreach ($ovMod as $key => $value) {
        ...
        $q = "INSERT INTO learningToolSlides ($keys_string) VALUES ('$implodeArray')";
	...
	$result = $this->connection->RunQuery("Insert into learningToolSlides (learningToolid, slideid, slideorder) values (\"$this->learningToolid\", \"$this->slideid\", \"$this->slideorder\")");
return $result; 	}
	...
	public function GetKeysOrderBy($column, $order){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT id from learningToolSlides order by $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
				$keys[$i] = $row["id"];
				$i++;
			}
     ...
     */
	public function endlearningToolSlides(){
