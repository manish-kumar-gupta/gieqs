...
Class questionsTag {
     ...
     *
     */
	public function New_questionsTag($questionid,$tagid){
	...
	public function Load_from_key($key_row){
		$result = $this->connection->RunQuery("Select * from questionsTag where id = \"$key_row\" ");
	...
	public function Delete_row_from_key($key_row){
		$result = $this->connection->RunQuery("DELETE FROM questionsTag WHERE id = $key_row");
return $result; 	}
	...
	public function Save_Active_Row(){
			...
			$q = "UPDATE questionsTag SET $implodeArray WHERE id = \"$this->id\"";
	...
	public function Save_Active_RowNulls(){
		$result = $this->connection->RunQuery("UPDATE questionsTag set questionid = \"$this->questionid\", tagid = \"$this->tagid\" where id = \"$this->id\"");
	return $result; 
	}
	...
	public function Save_Active_Row_as_New(){
	 ...
	 foreach ($ovMod as $key => $value) {
        ...
        $q = "INSERT INTO questionsTag ($keys_string) VALUES ('$implodeArray')";
	...
	$result = $this->connection->RunQuery("Insert into questionsTag (questionid, tagid) values (\"$this->questionid\", \"$this->tagid\")");
return $result; 	}
	...
	public function GetKeysOrderBy($column, $order){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT id from questionsTag order by $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
				$keys[$i] = $row["id"];
				$i++;
			}
     ...
     */
	public function endquestionsTag(){
