...
Class tags {
     ...
     *
     */
	public function New_tags($name,$catgory){
		$this->name = $name;
	...
	public function Load_from_key($key_row){
		$result = $this->connection->RunQuery("Select * from tags where id = \"$key_row\" ");
		while($row = $result->fetch_array(MYSQLI_ASSOC)){
			$this->id = $row["id"];
			$this->name = $row["name"];
			$this->catgory = $row["catgory"];
		}return $result; 
	}
	...
	public function Delete_row_from_key($key_row){
		$result = $this->connection->RunQuery("DELETE FROM tags WHERE id = $key_row");
return $result; 	}
	...
	public function Save_Active_Row(){
			...
			$q = "UPDATE tags SET $implodeArray WHERE id = \"$this->id\"";
	...
	public function Save_Active_RowNulls(){
		$result = $this->connection->RunQuery("UPDATE tags set name = \"$this->name\", catgory = \"$this->catgory\" where id = \"$this->id\"");
	return $result; 
	}
	...
	public function Save_Active_Row_as_New(){
	 ...
	 foreach ($ovMod as $key => $value) {
        ...
        $q = "INSERT INTO tags ($keys_string) VALUES ('$implodeArray')";
	...
	$result = $this->connection->RunQuery("Insert into tags (name, catgory) values (\"$this->name\", \"$this->catgory\")");
return $result; 	}
	...
	public function GetKeysOrderBy($column, $order){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT id from tags order by $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
				$keys[$i] = $row["id"];
				$i++;
			}
