...
Class images {
     ...
     *
     */
	public function New_images($url,$name,$type){
		$this->url = $url;
		$this->name = $name;
		$this->type = $type;
	}
	...
	public function Load_from_key($key_row){
		$result = $this->connection->RunQuery("Select * from images where id = \"$key_row\" ");
	...
	public function Delete_row_from_key($key_row){
		$result = $this->connection->RunQuery("DELETE FROM images WHERE id = $key_row");
return $result; 	}
	...
	public function Save_Active_Row(){
			...
			$q = "UPDATE images SET $implodeArray WHERE id = \"$this->id\"";
	...
	public function Save_Active_RowNulls(){
		$result = $this->connection->RunQuery("UPDATE images set url = \"$this->url\", name = \"$this->name\", type = \"$this->type\" where id = \"$this->id\"");
	return $result; 
	}
	...
	public function Save_Active_Row_as_New(){
	 ...
	 foreach ($ovMod as $key => $value) {
        ...
        $q = "INSERT INTO images ($keys_string) VALUES ('$implodeArray')";
	...
	$result = $this->connection->RunQuery("Insert into images (url, name, type) values (\"$this->url\", \"$this->name\", \"$this->type\")");
return $result; 	}
	...
	public function GetKeysOrderBy($column, $order){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT id from images order by $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
				$keys[$i] = $row["id"];
				$i++;
			}
