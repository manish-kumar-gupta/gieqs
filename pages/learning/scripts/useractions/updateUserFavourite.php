...
if ($users->matchRecord($userid)){
    ...
    if ($type == 1){
        ...
        $usersFavouriteVideo->setuser_id($userid);
        $usersFavouriteVideo->setvideo_id($videoid);
        $result = $usersFavouriteVideo->prepareStatementPDO();
        if ($result > 1){
        ...
        //$q = "DELETE FROM `usersFavouriteVideo` WHERE `user_id` = '$userid'";
        $result = $usersFavouriteVideo->deleteLink($userid, $videoid);
        if ($result){
...
$users->endusers();
$usersFavouriteVideo->endusersFavouriteVideo();