...
if (count($data) > 0){
	...
	if ($outputFormat == 9){
		...
		if ($data['query'] == 'deleteChapter'){
			
			$q = "DELETE FROM chapter where id = $table";
		...
		} if ($data['query'] == 'selectChapterSet'){
			
			//get chapter set with tags
			
			
			$q = "SELECT a.`id`, a.`split`, b.`id` as `chapterid`, b.`name`, b.`timeFrom`, b.`timeTo`, b.`number`, b.`name` AS `chaptername`, c.`id` AS `chapterTagid`, b.`description`, d.`tagName`, d.`id` AS `tags_id` FROM `video` as a INNER JOIN `chapter` as b ON a.`id` = b.`video_id` INNER JOIN `chapterTag` as c ON b.`id` = c.`chapter_id` INNER JOIN `tags` as d ON d.`id` = c.`tags_id` WHERE a.`id` = " . $table;
		...
		} if ($data['query'] == 'getTime'){
			
			//get chapter set with tags
			
			
			$q = "SELECT a.`id`, a.`split`, b.`id` as `chapterid`, b.`name`, b.`timeFrom`, b.`timeTo`, b.`number`, b.`name` AS `chaptername`, c.`id` AS `chapterTagid`, d.`tagName` FROM `video` as a INNER JOIN `chapter` as b ON a.`id` = b.`video_id` INNER JOIN `chapterTag` as c ON b.`id` = c.`chapter_id` INNER JOIN `tags` as d ON d.`id` = c.`tags_id` WHERE b.`id` = " . $table;
		...
		} if ($data['query'] == 'getTags'){
			
			//get chapter set with tags
			
			
			$q = "SELECT c.`id` AS `chapterTagid`, d.`tagName`, d.`id` FROM `video` as a INNER JOIN `chapter` as b ON a.`id` = b.`video_id` INNER JOIN `chapterTag` as c ON b.`id` = c.`chapter_id` INNER JOIN `tags` as d ON d.`id` = c.`tags_id` WHERE a.`id` = " . $table . ' GROUP BY d.`id` ORDER BY d.`tagName` ASC';
		...
		} if ($data['query'] == 'getChapterSelector'){
			...
			$q = "SELECT c.`url` FROM `imageSet` as a INNER JOIN `imageImageSet` as b ON a.`id` = b.`imageSet_id` INNER JOIN `images` as c on b.`image_id` = c.`id` WHERE a.`id` = $x";
			//echo $q;
