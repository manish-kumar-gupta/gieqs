...
Class chapterTag {
	...
	private $id; //int(10)
	private $chapterid; //int(10)
	private $tagid; //int(10)
	private $connection;

	public function chapterTag(){
     ...
     *
     */
	public function New_chapterTag($chapterid,$tagid){
	...
	public function Load_from_key($key_row){
		$result = $this->connection->RunQuery("Select * from chapterTag where id = \"$key_row\" ");
	...
	public function Delete_row_from_key($key_row){
		$result = $this->connection->RunQuery("DELETE FROM chapterTag WHERE id = $key_row");
return $result; 	}
	...
	public function Save_Active_Row(){
			...
			$q = "UPDATE chapterTag SET $implodeArray WHERE id = \"$this->id\"";
	...
	public function Save_Active_RowNulls(){
		$result = $this->connection->RunQuery("UPDATE chapterTag set chapterid = \"$this->chapterid\", tagid = \"$this->tagid\" where id = \"$this->id\"");
	return $result; 
	}
	...
	public function Save_Active_Row_as_New(){
	 ...
	 foreach ($ovMod as $key => $value) {
        ...
        $q = "INSERT INTO chapterTag ($keys_string) VALUES ('$implodeArray')";
	...
	$result = $this->connection->RunQuery("Insert into chapterTag (chapterid, tagid) values (\"$this->chapterid\", \"$this->tagid\")");
return $result; 	}
	...
	public function GetKeysOrderBy($column, $order){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT id from chapterTag order by $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
				$keys[$i] = $row["id"];
				$i++;
			}
