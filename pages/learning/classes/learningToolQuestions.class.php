...
Class learningToolQuestions {

	private $id; //int(10)
	private $learningToolid; //int(10)
	private $questionid; //int(10)
	private $connection;

	public function learningToolQuestions(){
     ...
     *
     */
	public function New_learningToolQuestions($learningToolid,$questionid){
	...
	public function Load_from_key($key_row){
		$result = $this->connection->RunQuery("Select * from learningToolQuestions where id = \"$key_row\" ");
	...
	public function Delete_row_from_key($key_row){
		$result = $this->connection->RunQuery("DELETE FROM learningToolQuestions WHERE id = $key_row");
return $result; 	}
	...
	public function Save_Active_Row(){
			...
			$q = "UPDATE learningToolQuestions SET $implodeArray WHERE id = \"$this->id\"";
	...
	public function Save_Active_RowNulls(){
		$result = $this->connection->RunQuery("UPDATE learningToolQuestions set learningToolid = \"$this->learningToolid\", questionid = \"$this->questionid\" where id = \"$this->id\"");
	return $result; 
	}
	...
	public function Save_Active_Row_as_New(){
	 ...
	 foreach ($ovMod as $key => $value) {
        ...
        $q = "INSERT INTO learningToolQuestions ($keys_string) VALUES ('$implodeArray')";
	...
	$result = $this->connection->RunQuery("Insert into learningToolQuestions (learningToolid, questionid) values (\"$this->learningToolid\", \"$this->questionid\")");
return $result; 	}
	...
	public function GetKeysOrderBy($column, $order){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT id from learningToolQuestions order by $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
				$keys[$i] = $row["id"];
				$i++;
			}
     ...
     */
	public function endlearningToolQuestions(){
