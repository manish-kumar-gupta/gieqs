...
Class learningTool {
	...
	private $active; //int(5)
	private $paid; //int(5)
	private $paidTier; //int(11)
	private $connection;

	public function learningTool(){
     ...
     *
     */
	public function New_learningTool($name,$active,$paid,$paidTier){
		$this->name = $name;
		$this->active = $active;
		$this->paid = $paid;
	...
	public function Load_from_key($key_row){
		$result = $this->connection->RunQuery("Select * from learningTool where id = \"$key_row\" ");
		while($row = $result->fetch_array(MYSQLI_ASSOC)){
			$this->id = $row["id"];
			$this->name = $row["name"];
			$this->active = $row["active"];
			$this->paid = $row["paid"];
			$this->paidTier = $row["paidTier"];
		}return $result; 
	}
	...
	public function Delete_row_from_key($key_row){
		$result = $this->connection->RunQuery("DELETE FROM learningTool WHERE id = $key_row");
return $result; 	}
	...
	public function Save_Active_Row(){
			...
			$q = "UPDATE learningTool SET $implodeArray WHERE id = \"$this->id\"";
	...
	public function Save_Active_RowNulls(){
		$result = $this->connection->RunQuery("UPDATE learningTool set name = \"$this->name\", active = \"$this->active\", paid = \"$this->paid\", paidTier = \"$this->paidTier\" where id = \"$this->id\"");
	return $result; 
	}
	...
	public function Save_Active_Row_as_New(){
	 ...
	 foreach ($ovMod as $key => $value) {
        ...
        $q = "INSERT INTO learningTool ($keys_string) VALUES ('$implodeArray')";
	...
	$result = $this->connection->RunQuery("Insert into learningTool (name, active, paid, paidTier) values (\"$this->name\", \"$this->active\", \"$this->paid\", \"$this->paidTier\")");
return $result; 	}
	...
	public function GetKeysOrderBy($column, $order){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT id from learningTool order by $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
				$keys[$i] = $row["id"];
				$i++;
			}
	...
	/**
	 * @return paidTier - int(11)
	 */
	public function getpaidTier(){
	 ...
	 * @param Type: int(11)
	 */
	public function setpaidTier($paidTier){
		$this->paidTier = $paidTier;
	}
     ...
     */
	public function endlearningTool(){
